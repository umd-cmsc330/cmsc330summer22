...
let concat x y =
  if x = Empty_String then y else if y = Empty_String then x else Concat (x, y)
...
let rec regex_gen n =
  ...
  match n with
  | 0 ->
      frequency [ (1, map epsilon char); (9, map symbol (char_range 'a' 'z')) ]
  | _ ->
      oneof
        [
...
let rec string_gen regex =
  ...
  | Concat (a, b) ->
      string_gen a >>= fun x ->
      string_gen b >>= fun y -> return (x ^ y)
  | Star s ->
      int_range 1 5 >>= fun n ->
      string_gen s >>= fun str -> return (dup str n)
...
let test_regex_to_nfa_accept_pbt =
  Test.make ~name:"regex_to_nfa_accept" ~count:100 (* number of tests *)
